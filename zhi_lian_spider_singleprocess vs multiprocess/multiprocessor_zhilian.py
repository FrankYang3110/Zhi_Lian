...
def get_company_detail(work_list, old_url_set,csv_filename):
        ...
        if not hashed_url in old_url_set:
            try:
                ...
                if re.status_code == 200:
                    ...
                    save_csv(csv_filename, work_dict)
...
def main():
    ...
    for i in range(page_count):
        ...
        for i in range(len(work_list)):
            dict_list[i%N].append(work_list[i])
        work_list.clear()
        pool = Pool()
        for i in dict_list:
            pool.apply_async(func=get_company_detail, args=(i, old_url_set, csv_filename))
        pool.close()
        pool.join()
...
if __name__ == '__main__':
    ...
    main()
    print('多进程用时：%s' % (time.time() - start_time))
